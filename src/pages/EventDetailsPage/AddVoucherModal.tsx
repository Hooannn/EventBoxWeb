...
import { DiscountType, IResponseData } from "../../types";
import {
  addToast,
  Button,
  DateRangePicker,
...
export default function AddVoucherModal(props: {
  isOpen: boolean;
  ...
  const { handleSubmit, control, reset, watch } = useForm<CreateInputs>({
    defaultValues: {
      discount_type: new Set<DiscountType>(["FIXED_AMOUNT"]),
    },
  });
  ...
  const createMutation = useMutation({
    mutationFn: (params: unknown) =>
      axios.post<IResponseData<boolean>>(
        `/v1/vouchers/event/${props.eventId}`,
        params
      ),
    onError,
    onSuccess(data) {
